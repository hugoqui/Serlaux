<%@ page language="c#" inherits="linetornet.textiles, App_Web_nswfhzkk" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN" >
<HTML>
	<HEAD>
		
		<title>Serlaux</title>
		<meta content="True" name="vs_showGrid">
		<meta content="http://schemas.microsoft.com/intellisense/ie5" name="vs_targetSchema">
		<LINK href="lin.css" type="text/css" rel="stylesheet">
		<link type="text/css" href="css/ui-lightness/jquery-ui-1.8.custom.css" rel="stylesheet" />	
		
		<link type="text/css" href="css/c/branding.css" rel="stylesheet" />
        <link type="text/css" href="css/c/business-header.css" rel="stylesheet" />
        <link type="text/css" href="css/c/colorbox.css" rel="stylesheet" />
        <link type="text/css" href="css/c/initial2.css" rel="stylesheet" />
        <link type="text/css" href="css/c/initial.css" rel="stylesheet" />
        <link type="text/css" href="css/c/reset.css" rel="stylesheet" />
        <link type="text/css" href="css/es/c/pri-nav.css" rel="stylesheet" />
		
		<script type="text/javascript" src="js/jquery-1.4.2.min.js"></script>
		<script type="text/javascript" src="js/jquery-ui-1.8.custom.min.js"></script>	
		
		
		
		
		<LINK href="lin.css" type="text/css" rel="stylesheet">
		<link href="print.css" type="text/css" media="print" rel="stylesheet">
	    <style type="text/css">
            .btn
            {}
            .style1
            {
                width: 500px;
            }
            .style2
            {
                width: 216px;
            }
        </style>
	</HEAD>
	<body class="index" id="accesorios-y-consumibles-para-banos-publicos-initial-directo">
    

    

    <div id="extraStyleWrapper" align="left">
      <div id="topWrapper">
        <header id="topHeader" class="centerThis">
          <div id="headerLeft">
            <header id="logoWrapper">
              
              <a href="/" title="Logo"><img id="logo" src="images/vans-header2.jpg" alt="Logo" style="padding-top: 28px;"></a>
              
            </header><!-- logoWrapper -->
          </div><!-- headerLeft -->
          <div id="headerRight">
             <div id ="siteextended" style="padding-right: 200px;">
               
                    <div class="rhs_calltoaction rhs_colour1 rhs_withshadow rhs_tel" style="width: 285px;" >
                        <h2="colour1"><span style="
    background-color: #0177c1;
    display: block;
    padding: 4px 15px 4px 18px;
    margin: 1em 0;
    background-repeat: no-repeat;
    background-position: 7px 3px;
    margin: 0;
    font-weight: bold;
    font-size: 1.2em;
        color: #fff;
">¿Podemos ayudarle?</span></h2>
                        <ul style="padding: 0 0 0 14px; font-size: 1.1em;">
                        <li class="phone"><h3 style="
    font-size: 24px;
">902 121 131</h3></li>
                        <li>Contacte con un representante de ventas</li>
                        </ul>
                        <div class="fade_divider"><hr /></div>
                    </div>
                </div>
          </div><!-- headerRight -->
          <div class="clearFix"></div>
        </header><!-- topHeader -->
        <nav id="navWrapper" class="">
          <!--#include file="Head2.inc" -->
        </nav><!-- navWrapper -->
      </div><!-- topWrapper -->
      <div id="contentWrapper">
        <div class="centerThis">
          <div id="content">

    <div id="billboardWrapper">
    <div id="billboard" class="theme-default nivoSlider">
    
    <div id="slider">
        <div><img src="images/comunidad1.jpg" style="display: inline; height: 388px; width: 948px;"></div>
        <div><img src="images/comunidad2.jpg" style="display: inline; height: 388px; width: 948px;"></div>
       <div><img src="images/comunidad3.jpg" style="display: inline; height: 388px; width: 948px;"></div>
    </div>
        <script type="text/javascript">
            $(function() {
                $('#slider div:gt(0)').hide();
                setInterval(function() {
                    $('#slider div:first-child').fadeOut(1000)
                        .next('div').fadeIn(1000)
                        .end().appendTo('#slider');
                }, 4000);
            })
        </script>
        
        
    
    <div class="nivo-caption" style="display: block;">
            <h3>Limpieza de comunidades</h3>
            <span></span>
        </div>
        
        
      
</div><!-- billboardWrapper -->



    <div id="contentSections" class="group" style="padding-left: 50;">
    	<div id="welcomeCol" class="floatLeft">
            
            <section id="welcomeContent">
                
                <div class="textile">
                    <div class="welcome-text">
<div class="welcome-upper">

</br>
<p>Sin lugar a dudas la limpieza de Comunidades de Propietarios es uno de los servicios donde más se contrata a empresas de limpieza. La razón es muy clara y sencilla, se requiere un alto grado de especialización y profesionalidad en la limpieza de este tipo de viviendas de Propiedad Horizontal. </p>

</div>
<div class="welcome-lower">



<p>Una de las cuestiones a tener en cuenta, respecto de los servicios de limpieza y mantenimiento de las Comunidades de Propietarios, es que deben respetar a las múltiples necesidades que las zonas comunes originan para cada uno de los vecinos, creando una sensación de satisfacción para la comunidad como colectivo único. </p>


</div>
</div>
                </div><!-- textile -->
            </section><!-- welcomeContent -->
            
    	</div><!-- welcomeCol -->
    	<div id="middleCol" class="floatLeft">
    	
    	<section id="welcomeContent">
                
                <div class="textile">
                    <div class="welcome-text">
<div class="welcome-upper">

</br>
<p>La limpieza de Comunidades de vecinos necesita de una adecuada coordinación de una multiplicidad de servicios como son la limpieza de fachadas, de cristales y ventanas, de garajes, de fachadas, de piscinas y zonas deportivas, zonas ajardinadas, recogida y gestión de residuos, entre otros.</p>


<p> Nuestro equipo cualificado trabaja para lograr resultados óptimos en la realización de las múltiples actividades objeto de contratación, como por alcanzar la satisfacción de todos y cada uno de los vecinos y de la Comunidad en su conjunto. </p>


</div>

</div>
                </div><!-- textile -->
            </section>
    	
         
          <!-- add bracket if needed 
		% include settings.middleContent % 
			   add closing bracket -->
    	</div><!-- middleCol -->
    	<div id="sidebar" class="floatRight">
            <span id="topBar">&nbsp;</span><!-- topBar -->
    		<div id="sidebarBkgd">
                <!-- facebookLike -->
	
            </div><!-- sidebarBkgd -->    
    	</div><!-- rightCol -->				
    </div><!-- contentSections -->


</div><!-- content -->
        </div><!-- centerThis -->
      </div><!-- contentWrapper -->
    </div><!-- extraStyleWrapper -->
 

    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.min.js"></script>
   
    

    

    
    </body>
</HTML>
